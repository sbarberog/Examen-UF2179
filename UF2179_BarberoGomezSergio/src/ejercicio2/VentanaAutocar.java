...
public class VentanaAutocar extends JFrame {
	...
	public VentanaAutocar() {
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"Matr\u00EDcula", "Marca", "Modelo", "Kil\u00F3metros", "Plazas"
			}
		) {
	...
	protected void insertarDatos() {
		...
		try {
			...
			if(matricula.isBlank() || marca.isBlank() || modelo.isBlank() || kilometros<0) {
				JOptionPane.showMessageDialog(contentPane, "Debe rellenar todos los campos", "Faltan datos", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if(!listaAutocares.contains(a)) {
				...
				JOptionPane.showMessageDialog(contentPane, "El dato se ha introducido correctamente", "Datos introducidos", JOptionPane.INFORMATION_MESSAGE);
			}else {
				JOptionPane.showMessageDialog(contentPane, "El autocar con matrícula "+matricula+" ya existe", "El autocar ya existe", JOptionPane.ERROR_MESSAGE);
			}
			
		} catch(NumberFormatException n) {
			JOptionPane.showMessageDialog(contentPane, "Debe introducir un número válido en el campo de kilómetros", "Dato inválido", JOptionPane.ERROR_MESSAGE);
		}
	}
